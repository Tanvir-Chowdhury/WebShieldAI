  ...
  <div class="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50">
    <div class="bg-[#1f2937] rounded-xl shadow-2xl p-8 w-[600px] relative">
      ...
      <h2 class="text-xl font-semibold mb-4 text-center text-white">
        Enable Dom Protection
      </h2>
      <p class="text-sm text-gray-400 mb-3 text-center">
        Copy and paste the following script into the <strong>&lt;head&gt;</strong> section of your website:
      </p>
...
import { ref } from "vue";
import api from "../../composables/axios"; // Adjust the import path if needed
...
async function checkCDNCode() {
  ...
  const scriptToCheck = `<script src="http://127.0.0.1:8000/cdn/dom-defacement-agent.js?wid=${props.websiteId}"><\/script>`;
  ...
  try {
    ...
    if (response.data.success) {
      ...
      alert("Dom protection script not found on the website.");
    }
  } catch (err) {
    alert("Error while checking for dom script.");
    console.error(err);
  } finally {
