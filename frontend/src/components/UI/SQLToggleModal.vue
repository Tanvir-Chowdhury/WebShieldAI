...
async function checkCDNCode() {
  ...
  const scriptToCheck = `<script src="http://127.0.0.1:8000/cdn/webshield-sql-agent.js?wid=${props.websiteId}"><\/script>`;
  ...
  try {
    ...
    if (response.data.success) {
      ...
      alert("SQL protection script not found on the site.");
    }
  } catch (err) {
    alert("Error checking SQL protection script.");
    console.error(err);
  } finally {
