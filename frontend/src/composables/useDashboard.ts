...
export function useDashboard() {
  ...
  const refreshSiteBlockedCount = async (websiteId: number) => {
    try {
      ...
      if (site) {
        site.metrics.blockedAttacks = Number(data?.total ?? 0);
      }
    } catch (e) {
      console.error(`Failed to load blocked count for site ${websiteId}:`, e);
    }
  };

  const refreshAllSitesBlockedCounts = async () => {
    await Promise.all(
      websites.value.map((w) => refreshSiteBlockedCount(toNum(w.id)))
    );
  };
  ...
  const refreshSiteMetrics = async (websiteId: number, days = 7) => {
    try {
      ...
      if (site) {
        ...
        site.metrics.responseTime = data?.p95_ms ?? data?.avg_ms ?? 0;
      }
    } catch (e) {
  ...
  const refreshGlobalBlockedCount = async () => {
    try {
      ...
      totalBlockedCount.value = Number(data?.total ?? 0);
    } catch (e) {
      console.error("Failed to load global blocked count:", e);
      totalBlockedCount.value = 0;
    }
  };
  ...
  const securityMetrics = computed<SecurityMetrics>(() => {
    const totals = { totalWebsites: websites.value.length, activeThreats: 0 };
  ...
  const loadWebsitesFromDB = fetchWebsitesFromDB;
  ...
  return {
    ...
    refreshGlobalBlockedCount,
    refreshSiteBlockedCount,
    refreshAllSitesBlockedCounts,
  };
}
